...
class TestCloseGripperEnv(Env):
    ...
    def _set_gripper_pos(self, value):
        """Set the gripper to a given position between closed (0.0) and open (1.0)"""
        gripper_dict = self.config["gripper"]
        lower_limit = gripper_dict["lower_limit"]
        upper_limit = gripper_dict["upper_limit"]

        pos = max(lower_limit, min(value, upper_limit))

        diff = upper_limit - lower_limit
        scaled_diff = diff * pos

        robot_pos = lower_limit + scaled_diff

        self.robot_interface.set_gripper_to_value(robot_pos)
